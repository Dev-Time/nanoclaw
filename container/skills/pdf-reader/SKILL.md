---
name: pdf-reader
description: Read and extract text from PDF files — documents, reports, contracts, spreadsheets. Use whenever you need to read PDF content, not just when explicitly asked. Handles local files, URLs, and channel attachments.
...
## Channel PDF attachments

When a user sends a PDF via a messaging channel (Telegram, WhatsApp, etc.), it is automatically saved to the `attachments/` directory. The message will include a path hint like:

...
### 2. IDLE_TIMEOUT and CONTAINER_TIMEOUT (now distinct)
Previously both defaults were 30 min, causing containers to always exit via hard SIGKILL (code 137). We have increased the defaults: **CONTAINER_TIMEOUT is now 1 hour (3600000 ms)** and **IDLE_TIMEOUT is now 55 min (3300000 ms)**. This 5-minute gap ensures that idle containers wind down gracefully before the hard safety kill fires.
